import { createApiClient } from '@mockery/api-client';
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  initSession: async (apiBaseUrl: string) => {
    const saved = sessionStorage.getItem('mockery_refresh_token');
    if (!saved) {
      set({ status: 'login' });
      return;
    }
    ...
    try {
      ...
      if (res.data) {
        ...
        sessionStorage.removeItem('mockery_refresh_token');
        set({ status: 'login' });
      }
    } catch {
      sessionStorage.removeItem('mockery_refresh_token');
  ...
  login: async (apiBaseUrl, email, password, clientType) => {
    ...
    const data = res.data!;
    sessionStorage.setItem('mockery_refresh_token', data.session.refreshToken);
  ...
  register: async (apiBaseUrl, name, email, password) => {
    ...
    sessionStorage.setItem('mockery_refresh_token', res.data!.session.refreshToken);
    set({
