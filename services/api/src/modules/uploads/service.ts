...
import type { StoragePaths } from "@mockery/storage";
import { createUploadRawPath, writeFileToPath } from "@mockery/storage";
...
export class UploadsService {
  ...
  private async requireCurrentUser(headers: UploadHeaders) {
    const session = await resolveSession(this.options, headers);
    return session ?? new UploadUnauthorizedError();
  }
}
